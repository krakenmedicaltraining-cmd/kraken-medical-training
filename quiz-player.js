...
(() => {
  ...
  async function waitForCoursePlayer() {
    ...
    console.error(
      "Quiz engine timed out while waiting for the course player."
    );
  ...
  function passedHtml() {
    return `
      <div class="quiz-result passed">
        ...
        <p>
          Your course is complete and your
  ...
  function attemptLimitReached() {
    const limit =
      Number(
        quizState.quiz?.max_attempts || 0
      );

    return (
      limit > 0 &&
      quizState.attemptCount >= limit &&
      !quizState.passed
    );
  }
  ...
  async function submitAttempt(
    ...
    if (passed) {
      await completeCourseAndIssueCertificate(
        score
      );
    }
  ...
  async function completeCourseAndIssueCertificate(
    score
  ) {
    const state =
      getPlayerState();
    ...
    */
    ...
    if (
      ...
      console.warn(
        "Course completion could not be updated:",
        progressResult.error
      );
    }
    ...
    */
    ...
    /*
      ...
      3. Supabase Auth full_name
      4. Supabase Auth display_name
      5. Email
      6. Kraken Learner
    */
    ...
    if (
      ...
      console.warn(
        "Learner profile name lookup failed:",
        profileResult.error
      );
    } else if (
      profileResult.data
  ...
  function applyCertificateGate() {
    ...
    if (
      ...
      ensureCertificateForEligibleCourse()
        .catch(
          error =>
            console.warn(
              "Certificate self-heal failed:",
              error
            )
        );
    }
