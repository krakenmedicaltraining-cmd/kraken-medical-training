...
(() => {
  ...
  function setState(text){
    ...
    if ($("#mobileState")) $("#mobileState").textContent=text;
  }
  ...
  function blank(type){
    return {client_id:uid(),type,heading:"",text:"",url:"",caption:"",button_text:"Open",items:""};
  }
  ...
  function blockHtml(b,i){
    ...
    return `<article class="block" data-i="${i}">
      <div class="blockhead">
        ...
        <button type="button" data-copy="${i}">⧉</button>
  ...
  function payload(){
    ...
    return {
      ...
      published_at:$("#publishedAt").value
        ...
        : ($("#status").value==="published"?new Date().toISOString():null),
  ...
  function renderItems(){
    ...
    const filtered=items.filter(x=>
      `${x.title} ${x.category} ${x.content_type} ${x.status}`.toLowerCase().includes(q)
      &&(!f||x.content_type===f)
    );

    $("#itemList").innerHTML=filtered.length
      ? filtered.map(x=>`
  ...
  async function edit(id){
    ...
    $("#itemId").value=x.id;
    $("#title").value=x.title||"";
    $("#excerpt").value=x.excerpt||"";
    $("#type").value=x.content_type||"news";
    $("#category").value=x.category||"";
    $("#author").value=x.author||"";
    $("#cover").value=x.cover_image_url||"";
    $("#media").value=x.media_url||"";
    $("#slug").value=x.slug||"";
    $("#status").value=x.status||"draft";
    $("#readingTime").value=x.reading_time_minutes||5;
    $("#eventDate").value=x.event_date?x.event_date.slice(0,16):"";
    $("#publishedAt").value=x.published_at?x.published_at.slice(0,16):"";
    $("#featured").checked=!!x.featured;
    $("#sharing").checked=x.allow_sharing!==false;
    $("#sendNewsletter").checked=false;

    blocks=(br.data||[]).map(b=>({...b,client_id:uid()}));

    $("#heading").textContent=`Editing: ${x.title}`;
  ...
  function reset(){
    ...
    $("#category").value="Kraken news";
    $("#readingTime").value=5;
    $("#sharing").checked=true;
    $("#status").value="draft";
    $("#type").value="news";
    $("#sendNewsletter").checked=false;
  ...
  async function sendNewsletter(itemId){
    setState("Publishing newsletter…");

    const { data, error } = await supabaseClient.functions.invoke(
      "send-newsletter",
      {
        body: { journal_item_id: itemId }
      }
    );
    ...
    toast(`Newsletter: ${sent} sent${skipped?`, ${skipped} skipped`:""}${failed?`, ${failed} failed`:""}`);
  }
  ...
  async function save(){
    ...
    if(!p.title||!p.excerpt){
      alert("Add a title and subtitle / summary.");
      return;
    }
    ...
    if($("#sendNewsletter").checked && p.status!=="published"){
      alert('To send this as a newsletter, set Status to "Published".');
      return;
    }

    if(shouldSend){
      const ok=confirm(
        "Publish this story AND email it to every active Kraken mailing-list subscriber?\n\n" +
        "The send log prevents the same story being emailed twice to the same subscriber."
      );
      if(!ok)return;
    }
    ...
    const d=await supabaseClient.from("journal_blocks").delete().eq("item_id",id);
    ...
    if(blocks.length){
      ...
      const ins=await supabaseClient.from("journal_blocks").insert(rows);
      if(ins.error)throw ins.error;
    }

    toast(currentId?"News story updated":"News story created");
    ...
    if(shouldSend){
      try{
        ...
        alert(
          "The news story was saved, but the newsletter could not be sent.\n\n" +
          (error.message||"Unknown error") +
          "\n\nYou can open the story again and retry."
        );
      }
    }
  ...
  async function duplicateExisting(id){
    ...
    currentId=null;
    $("#itemId").value="";
    $("#title").value=`${x.title} copy`;
    $("#slug").value="";
    $("#status").value="draft";
    $("#featured").checked=false;
    $("#sendNewsletter").checked=false;
    $("#heading").textContent="Duplicated story";
    $("#deleteItem").hidden=true;
    blocks=blocks.map(b=>({...structuredClone(b),id:undefined,client_id:uid()}));
    renderBlocks();
  ...
  async function deleteItem(id=currentId){
    ...
    if(!confirm(`Permanently delete "${x?.title||"this story"}"?`))return;
    if(prompt("Type DELETE to confirm.")!=="DELETE")return;

    const r=await supabaseClient.from("journal_items").delete().eq("id",id);
    if(r.error)throw r.error;

    toast("News story deleted");
    if(currentId===id)reset();
    await loadItems();
  }

  $$("[data-add]").forEach(b=>b.onclick=()=>add(b.dataset.add));
  $("#newItem").onclick=reset;
  ...
  $("#itemForm").addEventListener("input",e=>{
    if(e.target.id!=="title")setState("Unsaved changes");
  });
  ...
  (async()=>{
    ...
    reset();
    await loadItems();
  })().catch(e=>alert(`News builder could not start: ${e.message}`));
})();
